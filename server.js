...
let sock = null, isConnected = false, pendingQR = null;
...
async function connectWA() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') { isConnected = false; const code = (lastDisconnect && lastDisconnect.error instanceof Boom) ? lastDisconnect.error.output.statusCode : 0; if (code !== DisconnectReason.loggedOut) setTimeout(connectWA, 3000); }
  });
}
connectWA();
